...
// 上游 51ba3b0「保持画质的性能优化」：同帧阴影复用、共享实例缓冲与变化区间上传、
// 静止画面复用、AO 与景深共享深度。三个模块逐字节取自上游，未作改动。
...
export class ArchiveScene {
  ...
  /**
   * Reader / modal input lock. It only gates input: mode, selection, rotation
   * target, camera, quality and audio keep updating (CONTRACT.md §3, §13).
   */
  ...
  constructor(
    ...
    // 一帧内多个合成通道共用同一份几何：阴影只在主画面通道生成一次并复用。
    ...
    // 世界矩阵在每次模拟结束后统一更新一次；渲染通道复用同一批矩阵。
   ...
   * immersive reader so the background keeps updating while nothing underneath
   * responds to pointer, wheel or hover (CONTRACT.md §3, §13).
   */
  setInputSuspended(value: boolean) {
    if (this.inputSuspended === value) return;
    this.inputSuspended = value;
    this.cancelPointer();
    this.setHover(null);
    if (this.renderer?.domElement) {
      this.renderer.domElement.style.cursor = value ? "default" : this.canBrowse() ? "grab" : "default";
    }
  }
  get inputLocked() {
  ...
  resize() {
    ...
    // 只有 AO 与景深同分辨率时才能共享打包深度；降分辨率 AO 恢复原景深深度通道。
    this.ao.setSharing(
      this.ao.enabled &&
        this.bokeh.enabled &&
        this.quality.aoResolution === 1,
    );
  ...
  update(
    ...
    for (let i = 0; i < this.positions.length; i++) {
      ...
      this.themeUpdates?.scalar(i, this.theme.sample(this.cells[i], time));
    ...
    const instancesChanged = this.instances[0].count !== this.positions.length;
    const matricesChanged = this.matrixUpdates.commit();
    for (const inst of this.instances) inst.count = this.positions.length;
    // 拾取只用第一份实例表面；仅在变换或数量变化时重算包围球。
    if (matricesChanged || instancesChanged || !this.instances[0].boundingSphere)
    ...
    // 模拟与拾取继续每帧执行；只有画面输入完全一致时才复用上一帧合成结果。
    const state = this.renderState;
    this.scene.updateMatrixWorld();
    // 实例缓冲变化本身就说明画面变了：忙碌帧跳过材质快照，等画面稳定再采样。
    if (matricesChanged || cinematic) {
      ...
      this.scene.traverse((object) => {
        ...
        // 只跟踪应用侧写入的材质输入，不跟随 three 渲染期自增的计数器。
        ...
        state.floats(
          ...
          mat.color.g,
          mat.color.b,
          mat.attenuationColor?.r ?? 0,
          mat.attenuationColor?.g ?? 0,
          mat.attenuationColor?.b ?? 0,
        );
        for (const name of ["appearance", "glassClarity", "themeAmount"])
          state.floats(object.userData[name]?.value ?? 0);
        if (object instanceof THREE.InstancedMesh)
          state.add(
            object.count,
            object.instanceMatrix.version,
            this.themeAttribute?.version ?? 0,
          );
      });
      if (!state.end()) {
        this.reusedFrames++;
    ...
    this.renderedFrames++;
    // 主画面通道内生成一次阴影，后续 AO/景深通道复用同一份阴影贴图。
