...
> **本站移植说明（2026-09-13）**：本文来自上游 `LBEILC/RhineLabUI` 提交 `51ba3b0`（原 `codex/visual-preserving-performance` 分支），随该提交一并落入本仓库。上游的测量环境、宿主数据与截图指向上游机器（RTX 5070 Ti + Wallpaper Engine 原生宿主），**本站不能沿用其数字**。
>
> 本站适配差异：本站没有壁纸/HUD 线，也没有 `archive-visibility.ts`、`hud-projection.ts`、音乐律动与 `superPerformance`，因此
> ①「开场 HUD 布局缓存」「阵列候选列表复用」不适用；
> ② `reference/performance.ts` 基准页按本站 scene 接口改写（场景收敛为 idle/static/navigate/detail/inspect，去掉 `setPlayfield`/`setRhythmStyle`/`revealImmediately`/`uiOnlyParallax`）；
> ③ `scripts/build-performance.mjs` 改为 `configFile:false` 单独构建基准页，基线注入只替换 `src/scene.ts`。
>
> 本机复测（RTX 4060 Laptop、ANGLE/D3D11、无头 Chromium、1920×1080、每场景 24 帧）见 `.tools/upstream-sync/preview/perf-ab.json`：绘制调用 102→75（idle）/ 242→177（navigate），实例上传 93,312→18,400 字节/帧、静止后 0，三个场景逐像素 0 差异。真实 `/lab/` 回归见 `.tools/upstream-sync/preview/lab-port-report.json`：默认（呼吸动画中）持续重绘；开启「减少动态效果」后静止阵列与详情均 0 次重绘、全部复用。
